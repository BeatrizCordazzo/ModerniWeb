...
// If no cliente_id, try to resolve or create guest similar to create_custom_order
    ...
    $titulo = $data['title'] ?? ('Order - ' . date('Y-m-d H:i:s'));
    ...
    $colMap = [
        'cliente_id' => in_array('cliente_id', $availableCols) ? 'cliente_id' : null,
        'order_name' => in_array('order_name', $availableCols) ? 'order_name'
            : (in_array('titulo', $availableCols) ? 'titulo' : (in_array('title', $availableCols) ? 'title' : null)),
    ...
    if ($colMap['cliente_id']) {
        $insertCols[] = 'cliente_id'; $placeholders[] = '?'; $values[] = $cliente_id;
    }
    if ($colMap['order_name']) { $insertCols[] = $colMap['order_name']; $placeholders[] = '?'; $values[] = $titulo; }
    ...
    // If the orders table has the space columns, include them in INSERT
    ...
    // Insert pedido items into order_items table (if exists), otherwise store items as JSON in orders.items (best-effort)
