...
try {
    ...
    $rawFurniture = $data['furniture'] ?? $data['items'] ?? [];
    if (!is_array($rawFurniture)) {
        $rawFurniture = [];
    }

    if (!empty($rawFurniture) || !empty($data['images']) || !empty($data['spaceDimensions'])) {
