...
try {
    ...
    if (!$input || !isset($input['presupuesto_id']) || !isset($input['motivo'])) {
        throw new Exception('presupuesto_id and motivo are required');
    }

    $presupuestoId = intval($input['presupuesto_id']);
    $motivo = trim($input['motivo']);
    if ($motivo === '') {
        throw new Exception('Reason (motivo) is required');
    }
    ...
    if ($clientEmail && function_exists('mail')) {
        $subject = 'Notification: Quote rejected';
...
{
    $greeting = trim($name) !== '' ? "Hello {$name}," : 'Hello,';
    $projectText = $projectId ? " for project ID {$projectId}" : '';

    return "{$greeting}\n\n"
        . "Your quote (ID: {$presupuestoId}){$projectText} has been rejected.\n\n"
        . "Reason:\n{$motivo}\n\n"
        . "If you need more details, please reply to this message.\n\n"
        . "Moderni Team";
}
