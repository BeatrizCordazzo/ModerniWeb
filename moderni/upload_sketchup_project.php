...
if (!$userId || $userRole !== 'admin') {
    ...
    echo json_encode(['success' => false, 'error' => 'Only administrators can upload models..']);
    exit;
}

if (!isset($_FILES['skp_file'])) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'You must attach a file. .skp.']);
    exit;
}
...
if (!empty($file['error'])) {
    ...
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Could not upload the file.']);
    exit;
}
...
if ($extension !== 'skp') {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Only .skp files are allowed.']);
    exit;
}

// Size limit (150 MB for safety)
$maxSize = 150 * 1024 * 1024;
if (($file['size'] ?? 0) > $maxSize) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'The file exceeds the maximum allowed size (150 MB).']);
    exit;
}
...
if (!is_dir($uploadDir) && !@mkdir($uploadDir, 0775, true) && !is_dir($uploadDir)) {
    ...
    http_response_code(500);
    echo json_encode(['success' => false, 'error' => 'Could not prepare the file upload directory.']);
    exit;
}
...
try {
    ...
    http_response_code(500);
    echo json_encode(['success' => false, 'error' => 'Could not save the model.']);
    exit;
}
