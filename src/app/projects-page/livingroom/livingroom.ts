...
export class Livingroom implements OnInit, OnDestroy {
  ...
  loadProjects() {
    ...
    this.datosService.getLivingRoomProjects().subscribe({
      next: (projects) => {
        this.projects = projects;
        this.isLoading = false;
      },
      error: (error) => {
        console.error('Error loading living room projects:', error);
        this.errorMessage = 'Error al cargar los proyectos de sala de estar. Por favor, intenta de nuevo.';
        this.isLoading = false;
      }
    });
  }
